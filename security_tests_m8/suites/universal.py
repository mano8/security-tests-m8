...
def _admin_credentials() -> dict[str, str]:
    config = get_config()
    return {"username": config.admin_email, "password": config.admin_password}
...
class AuthAttackSuite:
    ...
    def test_a02_wrong_password_returns_400(self):
        r = requests.post(
            _LOGIN_URL,
            data={"username": get_config().admin_email, "password": "WRONG!"},
            timeout=TIMEOUT,
        )
    ...
    def test_a03_unknown_user_returns_400_not_404(self):
        ...
        assert r.status_code == 400, (
            f"[SECURITY FAIL-A03] Status {r.status_code} reveals user existence; "
    ...
    def test_a06_sql_injection_in_email_does_not_crash(self):
        ...
        assert r.status_code in (400, 422), (
            f"[SECURITY FAIL-A06] SQL injection caused unexpected "
    ...
    def test_a07_xss_in_email_not_reflected_unescaped(self):
        ...
        assert r.status_code in (400, 422)
        assert r.status_code != 500
        assert "<script>" not in r.text, "[SECURITY FAIL-A07] Reflected XSS payload"

    def test_a08_oversized_password_does_not_cause_bcrypt_dos(self):
        """Passwords >72 bytes fed to bcrypt can hang the process."""
        r = requests.post(
            _LOGIN_URL,
            data={"username": get_config().admin_email, "password": "A" * 10_000},
            timeout=TIMEOUT,
        )
        assert r.status_code != 500, (
            "[SECURITY FAIL-A08] Server crashed on oversized password (bcrypt DoS)"
        )
...
class AuthorizationSuite:
    ...
    def test_c03_regular_user_cannot_read_other_user(
        ...
        assert r.status_code == 403, (
            f"[SECURITY FAIL-C03] IDOR: regular user accessed user/{other_id}/"
        )
    ...
    def test_c06_regular_user_cannot_get_another_users_session(
        ...
        assert r.status_code == 403, (
            f"[SECURITY FAIL-C06] IDOR: sessions accessible cross-user: {r.status_code}"
        )
...
class RateLimitingSuite:
    ...
    def test_d01_six_bad_logins_trigger_429(self):
        """Default login limit must rate-limit the sixth bad request."""
        ...
        assert 429 in statuses, (
            f"[SECURITY FAIL-D01] Rate limiting not triggered after 6 attempts — "
            f"statuses: {statuses}"
        )
    ...
    def test_d02_rate_limit_response_is_informative(self):
        ...
        for _ in range(6):
            ...
            if r.status_code == 429:
                ...
                ), f"[SECURITY FAIL-D02] 429 detail not informative: {body['detail']}"
                return
    ...
    def test_d03_xff_header_does_not_bypass_email_rate_limit(self):
        ...
        assert 429 in statuses, (
            "[SECURITY FAIL-D03] XFF rotation bypassed email rate limit"
        )
    ...
    def test_d04_different_emails_each_get_own_limit(self):
        ...
        assert all(s == 400 for s in results), (
            f"[SECURITY OBS-D04] Unexpected fresh-email statuses: {results}"
        )
    ...
    def test_d06_refresh_rotation_rate_limited(self, regular_user: dict):
        """Default refresh rotation limit must rate-limit repeated refresh.
        ...
        for _ in range(12):  # > DEFAULT_MAX_REQUESTS=10
            ...
            if r.status_code == 200:
                ...
                break
        assert 429 in statuses, (
            f"[SECURITY FAIL-D06] Refresh rate limit not triggered — "
            f"statuses: {statuses}"
        )
...
class CORSSuite:
    ...
    def test_e01_allowed_origin_receives_cors_header(self):
        ...
        assert acao in ("http://localhost:5173", "*"), (
            f"[SECURITY FAIL-E01] Expected ACAO for allowed origin, got: '{acao}'"
        )
    ...
    def test_e02_unknown_origin_not_reflected(self):
        ...
        acao = r.headers.get("access-control-allow-origin", "")
        assert acao != "*", (
            "[SECURITY FAIL-E02] Wildcard CORS allows credentialed requests"
        )
        assert "evil-attacker.com" not in acao, (
            f"[SECURITY FAIL-E02] Arbitrary origin reflected in ACAO: '{acao}'"
        )
    ...
    def test_e03_null_origin_not_granted_credentials(self):
        ...
        assert not (acao == "null" and creds == "true"), (
            "[SECURITY FAIL-E03] null origin with credentials=true enables iframe CSRF"
        )
...
class PrivateAPISuite:
    ...
    def test_f01_private_route_blocked_by_traefik(self):
        """SECURITY PASS: Traefik must return 404 for public /private/."""
        try:
            r = requests.post(
                _public_url("user/private/users/"),
                json=self._BODY,
                timeout=TIMEOUT,
        ...
        assert r.status_code == 404, (
            "[SECURITY FAIL: TRAEFIK] /user/private is publicly routed. "
            f"from auth-public-router in dynamic_conf.yml. "
            f"Got {r.status_code}, expected 404. "
            "Fix: add PathPrefix(`/user/private`) to the exclusion list "
            "and restart Traefik."
        )

    def test_f02_private_route_blocked_with_wrong_token(self):
        """SECURITY PASS: Traefik must block /private/ even with a token."""
        try:
            r = requests.post(
                _public_url("user/private/users/"),
                json=self._BODY,
    ...
    def test_f03_private_route_blocked_with_admin_jwt(self, admin_headers: dict):
        """SECURITY PASS: Traefik must block /private/ even with admin JWT."""
        body = {
            **self._BODY,
    ...
    def test_f04_private_api_secret_committed_to_repo_documents_finding(self):
        """
        OPT-IN SECRET EXPOSURE CHECK — PRIVATE_API_SECRET is configured.

        Passing status 404 means the public Traefik boundary still blocked the
        request; status 200 means the known secret reached the app and created
        a user through the public route, which is a critical breach.
        ...
        """
        private_api_secret = get_config().private_api_secret
        if private_api_secret is None:
            pytest.skip("Set LIVE_TEST_PRIVATE_API_SECRET to run this opt-in check")
        body = {
            **self._BODY,
        ...
        try:
            ...
            return
        print(
            f"\n[SECURITY FAIL-F04] PRIVATE_API_SECRET in repo. "
...
class MetricsAPISuite:
    ...
    def test_f2_01_metrics_blocked_by_traefik(self):
        """SECURITY PASS: Traefik must return 404 for public /metrics."""
        try:
            r = requests.get(
                _public_url("user/metrics"),
                timeout=TIMEOUT,
                verify=get_config().public_tls_verify,
            )
        except requests.exceptions.SSLError:
            return
        assert r.status_code == 404, (
            "[SECURITY FAIL: TRAEFIK] /user/metrics is publicly routed. "
            f"from auth-public-router in dynamic_conf.yml. "
            f"Got {r.status_code}, expected 404. "
            "Fix: add PathPrefix(`/user/metrics`) to the exclusion list "
            "and restart Traefik."
        )
    ...
    def test_f2_02_metrics_absent_from_openapi(self):
        ...
        assert not metrics_paths, (
            f"[SECURITY FAIL-F2-02] Metrics route exposed in OpenAPI: {metrics_paths}. "
...
class AvatarUrlSuite:
    ...
    def test_g02_bare_filename_rejected(self, regular_user: dict):
        ...
        assert r.status_code == 422, (
            "[SECURITY FAIL-G02] Bare filename accepted as avatar — should be URL only"
        )
    ...
    def test_g03_ftp_scheme_rejected(self, regular_user: dict):
        ...
        assert r.status_code == 422, (
            "[SECURITY FAIL-G03] ftp:// avatar URL accepted — only http/https allowed"
        )
    ...
    def test_g04_protocol_only_rejected(self, regular_user: dict):
        ...
        assert r.status_code == 422, (
            "[SECURITY FAIL-G04] Protocol-only URL accepted — host required"
        )
...
class InfoDisclosureSuite:
    ...
    def test_h03_404_does_not_reveal_internal_path(self):
        ...
        assert r.status_code == 404
        assert "/opt/" not in r.text, "[SECURITY FAIL-H03] Internal path exposed in 404"

    def test_h04_metrics_endpoint_requires_auth_or_absent(self):
        """OBSERVATION: unauthenticated metrics may be acceptable only internally."""
        r = requests.get(f"{AUTH_BASE}/metrics", timeout=TIMEOUT)
        if r.status_code == 200:
            print(f"\n[SECURITY OBS-H04] Unauthenticated /metrics: {r.text[:300]}")
    ...
    def test_h05_health_endpoint_detail_level(self):
        ...
        if infra_keys:
            print(f"\n[SECURITY OBS-H05] Health reveals infra details: {infra_keys}")
    ...
    def test_h07_response_does_not_expose_hashed_password(self, admin_headers: dict):
        ...
        for user in r.json().get("data", []):
            assert "hashed_password" not in user, (
                "[SECURITY FAIL-H07] hashed_password exposed in users list"
            )
...
class SecurityHeadersSuite:
    ...
    def test_k01_x_content_type_options_nosniff(self, resp_headers: dict):
        ...
        assert val.lower() == "nosniff", (
            f"[SECURITY FAIL-K01] X-Content-Type-Options missing or wrong: '{val}'"
        )
    ...
    def test_k02_x_frame_options_restricts_framing(self, resp_headers: dict):
        ...
        assert val.upper() in ("DENY", "SAMEORIGIN"), (
            f"[SECURITY FAIL-K02] X-Frame-Options missing or permissive: '{val}'"
        )
    ...
    def test_k03_server_header_not_verbose(self, resp_headers: dict):
        ...
        for tech in ("uvicorn", "python", "fastapi"):
            assert tech not in server.lower(), (
                f"[SECURITY FAIL-K03] Server header reveals technology: '{server}'"
            )

    def test_k04_no_x_powered_by(self, resp_headers: dict):
        powered = resp_headers.get("x-powered-by", "")
        assert powered == "", (
            f"[SECURITY FAIL-K04] X-Powered-By reveals technology: '{powered}'"
        )
    ...
    def test_k06_referrer_policy_set(self, resp_headers: dict):
        ...
        if not rp:
            print("\n[SECURITY OBS-K06] Referrer-Policy absent; review leakage risk")
...
class CookieSecuritySuite:
    ...
    def test_l01_refresh_token_cookie_present(self, login_resp: requests.Response):
        assert "refresh_token" in login_resp.cookies, (
            "[SECURITY FAIL-L01] No refresh_token cookie in login response"
        )
    ...
    def test_l02_refresh_token_cookie_is_httponly(self, login_resp: requests.Response):
        ...
        assert "httponly" in raw.lower(), (
            "[SECURITY FAIL-L02] refresh_token missing HttpOnly — XSS can steal it"
        )

    def test_l03_refresh_token_cookie_has_samesite_strict(
        self, login_resp: requests.Response
    ):
        """SameSite=Strict gives strong CSRF protection for auth cookies."""
        raw = login_resp.headers.get("set-cookie", "")
        assert "samesite" in raw.lower(), (
            "[SECURITY FAIL-L03] refresh_token missing SameSite attribute"
        )
        assert "samesite=strict" in raw.lower(), (
            "[SECURITY FAIL-L03] SameSite is not Strict; "
    ...
    def test_l04_refresh_token_not_exposed_in_response_body(
        ...
        assert cookie_val not in body_str, (
            "[SECURITY FAIL-L04] Refresh token value in response body"
        )
    ...
    def test_l05_access_token_not_set_as_cookie(self, login_resp: requests.Response):
        ...
        assert "access_token" not in login_resp.cookies, (
            "[SECURITY FAIL-L05] Access token set as cookie — vulnerable to CSRF"
        )
    ...
    def test_l06_hashed_password_not_in_profile_response(self, admin_headers: dict):
        ...
        assert "hashed_password" not in body, (
            "[SECURITY FAIL-L06] hashed_password exposed in /profile/get/me/"
        )
...
class ApiKeySuite:
    ...
    def test_m01_create_returns_plaintext_once(self, admin_key: dict):
        ...
        assert pt.startswith("ak_"), (
            f"[SECURITY FAIL-M01] Plaintext missing or wrong prefix: '{pt[:20]}'"
        )
        assert len(pt) > 10

    def test_m01b_create_returns_required_fields(self, admin_key: dict):
        for field in ("id", "name", "expires_at", "revoked", "created_at"):
            assert field in admin_key, f"[SECURITY FAIL-M01b] Missing field '{field}'"
    ...
    def test_m14_key_hash_not_in_creation_response(self, admin_key: dict):
        assert "key_hash" not in admin_key, (
            "[SECURITY FAIL-M14] key_hash exposed in creation response"
        )
    ...
    def test_m03_verify_returns_ratelimit_headers(self, admin_key: dict):
        ...
        assert "x-ratelimit-limit" in h, "[SECURITY FAIL-M03] X-RateLimit-Limit missing"
        assert "x-ratelimit-remaining" in h, (
            "[SECURITY FAIL-M03] X-RateLimit-Remaining missing"
        )
        assert "x-ratelimit-reset" in h, "[SECURITY FAIL-M03] X-RateLimit-Reset missing"
    ...
    def test_m04_remaining_decrements_on_successive_calls(self, admin_headers: dict):
        ...
        assert r2 == r1 - 1, f"[SECURITY FAIL-M04] Remaining stuck: {r1} -> {r2}"
    ...
    def test_m05_invalid_key_returns_401(self):
        ...
        assert r.status_code == 401, (
            f"[SECURITY FAIL-M05] Invalid key returned {r.status_code} instead of 401"
        )

    def test_m06_missing_header_returns_422(self):
        r = requests.get(f"{self._BASE}/verify", timeout=TIMEOUT)
        assert r.status_code == 422, (
            f"[SECURITY FAIL-M06] Missing X-API-Key returned {r.status_code} not 422"
        )
    ...
    def test_m07_plaintext_not_in_list_response(
        ...
        for key_obj in r.json():
            assert "plaintext" not in key_obj, (
                "[SECURITY FAIL-M07] plaintext exposed in list endpoint"
            )
    ...
    def test_m07b_plaintext_not_in_get_response(
        ...
        assert r.status_code == 200
        body = r.json()
        assert "plaintext" not in body, "[SECURITY FAIL-M07b] plaintext exposed in get"
    ...
    def test_m08_list_returns_only_own_keys(self, regular_user: dict, admin_key: dict):
        ...
        assert admin_key["id"] not in ids, (
            "[SECURITY FAIL-M08] IDOR: regular user can see admin's API key"
        )
    ...
    def test_m09_get_returns_404_for_other_users_key(
        ...
        assert r.status_code == 404, (
            f"[SECURITY FAIL-M09] User accessed admin key: {r.status_code}"
        )
    ...
    def test_m13_revoke_other_users_key_returns_404(
        ...
        assert r.status_code == 404, (
            f"[SECURITY FAIL-M13] IDOR: regular user revoked admin key: {r.status_code}"
        )
    ...
    def test_m11_revoked_key_returns_401(self, user_key: dict):
        ...
        assert r.status_code == 401, (
            f"[SECURITY FAIL-M11] Revoked key accepted: {r.status_code}"
        )
    ...
    def test_m12_double_revoke_returns_409(self, admin_headers: dict, user_key: dict):
        ...
        assert r.status_code in (404, 409), (
            f"[SECURITY FAIL-M12] Double-revoke returned {r.status_code} "
            "(expected 409 or 404)"
        )


_AUTH_MARKS = [pytest.mark.live, pytest.mark.live_security]
setattr(AuthAttackSuite, "pytestmark", _AUTH_MARKS)
setattr(JWTStructuralSuite, "pytestmark", _AUTH_MARKS)
setattr(AuthorizationSuite, "pytestmark", _AUTH_MARKS)
setattr(RateLimitingSuite, "pytestmark", [*_AUTH_MARKS, pytest.mark.destructive])
setattr(CORSSuite, "pytestmark", _AUTH_MARKS)
setattr(PrivateAPISuite, "pytestmark", _AUTH_MARKS)
setattr(MetricsAPISuite, "pytestmark", _AUTH_MARKS)
setattr(HealthAPISuite, "pytestmark", _AUTH_MARKS)
setattr(AvatarUrlSuite, "pytestmark", _AUTH_MARKS)
setattr(InfoDisclosureSuite, "pytestmark", _AUTH_MARKS)
setattr(SecurityHeadersSuite, "pytestmark", _AUTH_MARKS)
setattr(CookieSecuritySuite, "pytestmark", _AUTH_MARKS)
setattr(ApiKeySuite, "pytestmark", _AUTH_MARKS)
